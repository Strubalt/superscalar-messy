...
public class Processor extends Component {
            ...
            switch(instr.getOpCode()){
                case OpCodeTable.ADD:   //Ri = Rj + Rk
                    setResult(result, instr.getRegi(), 
                        regs[instr.getRegj()]+regs[instr.getRegk()], false);
                    break;
                ...
                case OpCodeTable.SUBI:  //Ri = Rj - x
                    setResult(result, instr.getRegi(), 
                        regs[instr.getRegj()]-instr.getImmediate(), false);
                    break;
                ...
                case OpCodeTable.LOAD:  //Ri=Mem[Rj+x]
                    ...
                    result.memoryAddress = regs[instr.getRegj()]+instr.getImmediate();
                ...
                case OpCodeTable.STORE: //Mem[Rj+x]=Rk
                    ...
                    result.memoryRW = false;
                    result.data = regs[instr.getRegk()];
                    break;
                ...
                case OpCodeTable.B:     //PC = Rj
                    pc = regs[instr.getRegj()];
                    pcAdd4=false;
                    break;
                case OpCodeTable.BEQZ:  //IF Rj=0  THEN PC=PC+x
                    if(regs[instr.getRegj()] == 0) {
        ...
        private boolean writeBack() {
            if(null != memAccessedInstruciton) {
                if(memAccessedInstruciton.requireWriteBack) {
                    this.regs[memAccessedInstruciton.writeBackRegister] =
	...
	int getReg(int registerNumber) {
		return this.regs[registerNumber];
	}
	
	void setReg(int registerNumber, int value) {
		this.regs[registerNumber] = value;
	}
	...
	void dumpRegs() {
		...
		for(int i=0; i<this.regs.length; ++i) {
			System.out.println("R" + new Integer(i).toString() + 
							" = " + new Integer(this.regs[i]).toString());
		}
